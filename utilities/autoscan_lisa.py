...
bvals = np.linspace(-15, 15, 151)
pol = 'mm'
name = 'testrun_mm'

timestamp = datetime.now().strftime('%Y-%m-%d/%H%M%S')

outfolder = os.environ['HOME'] + "/atreju/results/" + timestamp + '-' + name
...
for b in bvals:
  ...
  with open('../tmp/job-' + name, 'wb') as ifile:
    ifile.write('#PBS -lwalltime=10:00\n')
    ...
    ifile.write('echo "NProbe = 161," >> config.info\n')
    ifile.write('echo "NCoupling = 801," >> config.info\n')
    ifile.write('echo "ShiftProbe = -20," >> config.info\n')
    ifile.write('echo "ShiftCoupling = -100," >> config.info\n')
