...
for b in bvals:
  ...
  with open('../tmp/job-' + name, 'wb') as ifile:
    ...
    ifile.write('echo "NProbe = 1921," >> config.info\n')
    ifile.write('echo "NCoupling = 2041," >> config.info\n')
    ifile.write('echo "ShiftProbe = -240," >> config.info\n')
    ifile.write('echo "ShiftCoupling = -215," >> config.info\n')
